...
function updateOrders()
{
  ...
  for (var i = 0; i < values.length; i++) {
        poTotalAmount = values[i][10];
        tracking= values[i][19];
        afpoNumber= values[i][18];
        sellOrderId  = values[i][2];
  ...
  var url = "http://ordermanager.supplydistributor.com/api/autofulfill_update";
  var options = {
    "method": "post",
    "headers": {
...
function getOrders_(data)
{
  ...
  for(var a=0; a<orders.length; a++)
  {
    ...
    if(findCell(referenceNumber)==0)
      emails.push(['',storeName, referenceNumber, itemLink,ASIN,qty,maxPrice,itemPrice,'','', discountPayment,name,street1,street2,city,state,zipCode,phone]);
  }
...
function updateShipping(sellOrderId, trackingNumber, carrier) {
  ...
  for (var i = 0; i < values.length; i++) {
      ...
      if (values[i][2] == sellOrderId) {
         var col = i+1;
         sheet.getRange("U"+col).setValue(trackingNumber);
         sheet.getRange("V"+col).setValue(carrier);
         flag = true;
         break;
      }
