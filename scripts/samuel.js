...
function updateOrders()
{
  ...
  for (var i = 0; i < values.length; i++) {
        poTotalAmount = values[i][21];
        tracking= values[i][24];
        afpoNumber= values[i][18];
        sellOrderId  = values[i][3];
...
function getOrders_(data)
{
  ...
  for(var a=0; a<orders.length; a++)
  {
    ...
    if(findCell(referenceNumber)==0)
      emails.push(['','', date, referenceNumber,storeName,'',ASIN,maxPrice,qty, name,street1,street2,city,state,zipCode]);
  }
...
function updateShipping(sellOrderId, trackingNumber, carrier) {
  ...
  for (var i = 0; i < values.length; i++) {
      ...
      if (values[i][3] == sellOrderId) {
         var col = i+1;
         sheet.getRange("A"+col).setValue(trackingNumber);
         sheet.getRange("B"+col).setValue(carrier);
         flag = true;
         break;
      }
