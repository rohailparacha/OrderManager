...
use App\Exports\CindyBceExport;
use App\Exports\CindyCancelExport;
use Session;
use Redirect;
use Validator;
...
class orderFulfillmentController extends Controller
{
    ...
    {        
        ...
        $fileName = date("d-m-Y")."-".time()."-cindy-auto-fulfillment-orders.csv";  
        return Excel::download(new CindyBceExport(), $fileName);             
    }

    public function orderCancelledExport(Request $request)
    {        
        
        $fileName = date("d-m-Y")."-".time()."-cindy-cancelled-orders.csv";
        return Excel::download(new CindyCancelExport(), $fileName);             
    }
    ...
    public function storeSettings(Request $request)
    {
        ...
        if(empty($settings))
            ...
            'amountCheck'=>$pricecheck,'stores'=>json_encode($stores),'storesCheck'=>$storecheck, 'discount'=>$discount, 'maxPrice'=>$maxPrice ,'maxDailyOrder'=>$maxDailyOrder, 'maxDailyAmount'=>$maxDailyAmount,'dailyAmountCheck'=>$dailyamtcheck, 'dailyOrderCheck'=>$dailyordercheck, 'name'=>'cindy','priority'=>$priority,'enabled'=>$enabled,'listCheck'=>$showinlist]);
        else
