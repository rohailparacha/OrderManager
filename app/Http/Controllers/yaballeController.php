...
use App\Exports\YaballeExport;
use App\Exports\YaballeOrdersExport;
use DB;
use App\Exports\YaballeBceExport;
use App\Exports\YaballeCancelExport;
use App\Imports\YaballeImport;
use Session;
use Redirect;
use Validator;
...
class yaballeController extends Controller
{
    //
    ...
    {
        $stores = accounts::all();
        $settings = settings::where('name','yaballe')->get()->first();
        return view('yaballeSettings',compact('stores','settings'));
    }
    ...
    public function yaballeexport(Request $request)
    {
        ...
        return Excel::download(new YaballeExport($storeFilter,$marketFilter,$stateFilter,$amountFilter,$sourceFilter), $filename);
    }
    ...
    {        
        ...
        $fileName = date("d-m-Y")."-".time()."-yaballe-auto-fulfillment-orders.csv";  
        return Excel::download(new YaballeBceExport(), $fileName);      
    }

    public function orderCancelledExport(Request $request)
    {        
        
        $fileName = date("d-m-Y")."-".time()."-yaballe-cancelled-orders.csv";
        return Excel::download(new YaballeCancelExport(), $fileName);             
    }

    public function autofulfillProcessed()
    {    
        if(auth()->user()->role==1)
            $orders = orders::select()->where('status','processing')->where('account_id','Yaballe')->orderBy('date', 'ASC')->paginate(100);
        ...
        elseif(auth()->user()->role==2)
            {
                ...
                $orders = orders::select()->where('status','processing')->where('account_id','Yaballe')->whereIn('storeName',$strArray)->orderBy('date', 'ASC')->paginate(100);
            }
        
        else
            
            $orders = orders::select()->where('status','processing')->where('account_id','Yaballe')->where('uid',auth()->user()->id)->orderBy('date', 'ASC')->paginate(100);
        ...
        foreach ($orders as $order)
        {
            $bcecheck = orders::select()->where('converted',false)->where('account_id','Yaballe')
        ...
        return view('yaballe.processed',compact('orders'));
    ...
    public function autoFulfillFilter(Request $request)
    {
        ...
        if(auth()->user()->role==1)
            $orders = $orders->where('flag','17')->where('status','unshipped')->orderBy('date', 'ASC')->groupby('orders.id')->paginate(100);
        elseif(auth()->user()->role==2)
        {
            $stores = accounts::select()->where('manager_id',auth()->user()->id)->get(); 
                $strArray  = array();
                ...
                $orders = $orders->where('flag','17')->where('status','unshipped')->whereIn('storeName',$strArray)->orderBy('date', 'ASC')->paginate(100);
        }
            
        else
            $orders = $orders->where('flag','17')->where('status','unshipped')->where('uid',auth()->user()->id)->orderBy('date', 'ASC')->groupby('orders.id')->paginate(100);
        ...
        $maxPrice = ceil(orders::where('status','unshipped')->where('flag','17')->max('totalAmount'));
        foreach($orders as $order)
        {
        ...
        $flags= flags::all();
        return view('yaballe.new',compact('flags','orders','stateFilter','marketFilter','sourceFilter','storeFilter','amountFilter','stores','states','maxAmount','minAmount','maxPrice'));
    }
    ...
    {  
            if(auth()->user()->role==1)
            {
                $orders = orders::select()->where('status','unshipped')->orderBy('date', 'ASC')->where('flag','17')->paginate(100);
            }
            ...
            elseif(auth()->user()->role==2)
            {
                ...
                $orders = orders::select()->where('status','unshipped')->whereIn('storeName',$strArray)->where('flag','17')->orderBy('date', 'ASC')->paginate(100);
        ...
        $maxAmount = ceil(orders::where('status','unshipped')->where('flag','17')->max('totalAmount'));
        ...
        $flags= flags::all();
        return view('yaballe.new',compact('flags','orders','stores','states','maxAmount','minAmount','maxPrice'));
    }
    ...
    {       
        ...
        if(auth()->user()->role==1)            
        {
            $orders = orders::select()->where('converted',false)->where('account_id','Yaballe')
            ->where('marketPlace','Walmart')
            ->where('carrierName',$amzCarrier->id)
            ->where('status','processing')
            ->where('trackingNumber','like','TBA%')
        ...
        elseif(auth()->user()->role==2)
        {
            ...
            ->where('marketPlace','Walmart')
            ->where('carrierName',$amzCarrier->id)
            ->where('account_id','Yaballe')->whereIn('storeName',$strArray)
            ->where('status','processing')
            ->where('trackingNumber','like','TBA%')
            ->orderBy('status', 'DESC')->paginate(100);
        ...
        return view('yaballe.bce',compact('orders'));
    }
        ...
        if($request->hasFile('file'))
        {
            ...
            {                
           ...
           else
             {
                ...
                return redirect()->route('yaballenew');
             }
    ...
    {        
        ...
        return view('yaballe.cancel',compact('orders'));
    }
    ...
    public function deleteCancelled($id)
    {
        ...
        if($order->poNumber != $order->afpoNumber)
        {
            ...
            return redirect()->route('yaballecancel')->withStatus(__('Order successfully deleted.'));
        }
        else
        {
            return redirect()->route('yaballecancel')->withStatus(__('Order could not be deleted.'));
        }
    ...
    public function deleteConversion($id)
    {
        ...
        if($order)
        {             
            return redirect()->route('yaballebce')->withStatus(__('Order successfully deleted.'));
        }
        else
        {
            return redirect()->route('yaballebce')->withStatus(__('Order could not be deleted.'));
        }
    ...
    public function storeSettings(Request $request)
    {
        ...
        if(empty($settings))
            ...
            'amountCheck'=>$pricecheck,'stores'=>json_encode($stores),'storesCheck'=>$storecheck, 'discount'=>$discount, 'maxPrice'=>$maxPrice ,'maxDailyOrder'=>$maxDailyOrder, 'maxDailyAmount'=>$maxDailyAmount,'dailyAmountCheck'=>$dailyamtcheck, 'dailyOrderCheck'=>$dailyordercheck,'name'=>'yaballe','priority'=>$priority,'enabled'=>$enabled,'listCheck'=>$showinlist]);
        else
            settings::where('name','yaballe')->where('id',$settings->id)->update(['minAmount'=>$minAmount,'maxAmount'=>$maxAmount,
            'quantityRangeCheck'=>$qtyrangecheck,'minQty'=>$minQty,'maxQty'=>$maxQty,'amountCheck'=>$pricecheck,'stores'=>json_encode($stores),'storesCheck'=>$storecheck, 'discount'=>$discount, 'maxPrice'=>$maxPrice,'maxDailyOrder'=>$maxDailyOrder, 'maxDailyAmount'=>$maxDailyAmount,'dailyAmountCheck'=>$dailyamtcheck, 'dailyOrderCheck'=>$dailyordercheck,'name'=>'yaballe','priority'=>$priority,'enabled'=>$enabled,'listCheck'=>$showinlist]);

        Session::flash('success_msg', __('Settings successfully updated'));
        return redirect()->route('yaballeSetting');
    ...
    public function search(Request $request)
    {
        ...
        {            
            ...
            {            
                ...
                $orders = orders::select()->where('flag','17')->where('status','unshipped')                
            ...
            elseif(auth()->user()->role==2)
            {
                ...
                $orders = orders::select()->where('flag','17')->where('status','unshipped')->whereIn('storeName',$strArray)
                ->where(function($test) use ($query){
                    $test->where('sellOrderId', 'LIKE', '%'.$query.'%');
                    $test->orWhere('buyerName', 'LIKE', '%'.$query.'%');
                })                
            ...
            {
            $orders = orders::select()->where('flag','17')->where('status','unshipped')->where('uid',auth()->user()->id)
            ->where(function($test) use ($query){
                $test->where('sellOrderId', 'LIKE', '%'.$query.'%');
                $test->orWhere('buyerName', 'LIKE', '%'.$query.'%');
            })                
            ->orderBy('date', 'ASC')->paginate(100);
                ...
                $flags= flags::all();
                return view('yaballe.new',compact('flags','orders','stores','states','maxAmount','minAmount','maxPrice','search','route'));
            
        }

        else if ($route=='yaballecancel')
        {
            if(auth()->user()->role==1)            
        {
            $orders = cancelled_orders::leftJoin('orders','cancelled_orders.order_id','=','orders.id')
        ...
        elseif(auth()->user()->role==2)
        {
            ...
            $orders = $orders->appends('searchQuery',$query)->appends('route', $route);
            return view('yaballe.cancel',compact('orders','search','route'));
        }        

        else if ($route=='yaballebce')
        {
        ...
        elseif(auth()->user()->role==2)
        {
            ...
            $orders = $orders->appends('searchQuery',$query)->appends('route', $route);
            return view('yaballe.bce',compact('orders','search','route'));
        }  

        else if ($route=='yaballeprocessed')
        {
            if(auth()->user()->role==1)
            ...
            elseif(auth()->user()->role==2)
                {
                    ...
                    ->where('account_id','Yaballe')->whereIn('storeName',$strArray)->orderBy('date', 'ASC')->paginate(100);
                }
                ...
                ->where('account_id','Yaballe')->where('uid',auth()->user()->id)->orderBy('date', 'ASC')->paginate(100);
            ...
            $orders = $orders->appends('searchQuery',$query)->appends('route', $route);
            return view('yaballe.processed',compact('orders','search','route'));
        }  
        elseif($route=='yaballereturn')
        {
            ...
            return view('yaballe.return',compact('returns','accounts','stores','dateRange','search','route'));
            
            
        }
        elseif($route=='yaballerefund')
        {
            if(auth()->user()->role==1)
            {
                $returns = returns::leftJoin('orders','orders.id','=','returns.order_id')
            ...
            $dateRange = $from .' - ' .$to;
            $returns = $returns->appends('searchQuery',$query)->appends('route', $route);
            return view('yaballe.refund',compact('returns','accounts','stores','dateRange','search','route'));
        }
        elseif($route=='yaballecompleted')
        {
            if(auth()->user()->role==1)
            {
                $returns = returns::leftJoin('orders','orders.id','=','returns.order_id')
            ...
            $dateRange = $from .' - ' .$to;
            $returns = $returns->appends('searchQuery',$query)->appends('route', $route);
            return view('yaballe.complete',compact('returns','accounts','stores','dateRange','search','route'));
        }
