...
class saleFreaks3Controller extends Controller
{
    //
    ...
    {
        $stores = accounts::all();
        $settings = settings::where('name','salefreaks3')->get()->first();
        return view('saleFreaks3Settings',compact('stores','settings'));
    }
    ...
    public function saleFreaks3export(Request $request)
    {
        ...
        return Excel::download(new SaleFreaksExport($storeFilter,$marketFilter,$stateFilter,$amountFilter,$sourceFilter, $daterange,'24'), $filename);
    }

    public function saleFreaks3OrderExport(Request $request)
    {        
        $filename = date("d-m-Y")."-".time()."-autofulfill-orders.csv";
        return Excel::download(new SaleFreaksOrdersExport('24'), $filename);
    }
    ...
    {    
        if(auth()->user()->role==1)
            $orders = orders::select()->where('status','processing')->where('account_id','SaleFreaks3')->orderBy('date', 'ASC')->paginate(100);
        ...
        elseif(auth()->user()->role==2)
            {
                ...
                $orders = orders::select()->where('status','processing')->where('account_id','SaleFreaks3')->whereIn('storeName',$strArray)->orderBy('date', 'ASC')->paginate(100);
            }
        
        else
            
            $orders = orders::select()->where('status','processing')->where('account_id','SaleFreaks3')->where('uid',auth()->user()->id)->orderBy('date', 'ASC')->paginate(100);
        ...
        foreach ($orders as $order)
        {
            $bcecheck = orders::select()->where('converted',false)->where('account_id','SaleFreaks3')
        ...
        return view('SaleFreaks3.processed',compact('orders'));
    ...
    public function autoFulfillFilter(Request $request)
    {
        ...
        if(auth()->user()->role==1)
            $orders = $orders->where('flag','24')->where('status','unshipped')->orderBy('assignDate', 'ASC')->groupby('orders.id')->paginate(100);
        elseif(auth()->user()->role==2)
        {
            $stores = accounts::select()->where('manager_id',auth()->user()->id)->get(); 
                $strArray  = array();
                ...
                $orders = $orders->where('flag','24')->where('status','unshipped')->whereIn('storeName',$strArray)->orderBy('assignDate', 'ASC')->paginate(100);
        }
            
        else
            $orders = $orders->where('flag','24')->where('status','unshipped')->where('uid',auth()->user()->id)->orderBy('assignDate', 'ASC')->groupby('orders.id')->paginate(100);
        ...
        $maxPrice = ceil(orders::where('status','unshipped')->where('flag','24')->max('totalAmount'));
        foreach($orders as $order)
        {
        ...
        $flags= flags::select()->whereNotIn('id',['16','24','8','9','10'])->get();  
        $route = 'saleFreaks3new';
        return view('SaleFreaks3.new',compact('flags','orders','stateFilter','marketFilter','sourceFilter','storeFilter','amountFilter','stores','states','maxAmount','minAmount','maxPrice','dateRange','route'));
    }
    ...
    {  
            if(auth()->user()->role==1)
            {
                $orders = orders::select()->where('status','unshipped')->orderBy('assignDate', 'ASC')->where('flag','24')->paginate(100);
            }
            ...
            elseif(auth()->user()->role==2)
            {
                ...
                $orders = orders::select()->where('status','unshipped')->whereIn('storeName',$strArray)->where('flag','24')->orderBy('assignDate', 'ASC')->paginate(100);
        ...
        $maxAmount = ceil(orders::where('status','unshipped')->where('flag','24')->max('totalAmount'));
        ...
        $flags= flags::select()->whereNotIn('id',['16','24','8','9','10'])->get();  
        $startDate = orders::where('status','unshipped')->where('flag','24')->min('assignDate');
        $endDate = orders::where('status','unshipped')->where('flag','24')->max('assignDate');

        $from = date("m/d/Y", strtotime($startDate));  
        $to = date("m/d/Y", strtotime($endDate));  
        $dateRange = $from .' - ' .$to;
        return view('SaleFreaks3.new',compact('flags','orders','stores','states','maxAmount','minAmount','maxPrice','dateRange'));
    }
    ...
    {       
        ...
        if(auth()->user()->role==1)            
        {
            $orders = orders::select()->where('converted',false)->where('account_id','SaleFreaks3')
            ->where('marketPlace','Walmart')
            ->where('carrierName',$amzCarrier->id)
            ->where('status','processing')
            ->where('trackingNumber','like','TBA%')
        ...
        elseif(auth()->user()->role==2)
        {
            ...
            ->where('marketPlace','Walmart')
            ->where('carrierName',$amzCarrier->id)
            ->where('account_id','SaleFreaks3')->whereIn('storeName',$strArray)
            ->where('status','processing')
            ->where('trackingNumber','like','TBA%')
            ->orderBy('status', 'DESC')->paginate(100);
        ...
        return view('SaleFreaks3.bce',compact('orders'));
    }
    ...
    public function updateSheetTracking($tracking, $sellOrderId, $carrier)
    {
        ...
        try{
            ...
            $client = new client(); 
            $endPoint = env('SALEFREAKS3_TOKEN', '');
    ...
    public function autoFulfillProcess(Request $request)
    {
        ...
        if($request->hasFile('file'))
        {
            ...
            {                
           ...
           else
             {
                ...
                return redirect()->route('saleFreaks3new');
             }
        ...
        $import = new SaleFreaksImport('24');
    ...
    {        
        ...
        return view('SaleFreaks3.cancel',compact('orders'));
    }
    ...
    public function deleteCancelled($id)
    {
        ...
        if($order->poNumber != $order->afpoNumber)
        {
            ...
            return redirect()->route('saleFreaks3cancel')->withStatus(__('Order successfully deleted.'));
        }
        else
        {
            return redirect()->route('saleFreaks3cancel')->withStatus(__('Order could not be deleted.'));
        }
    ...
    public function deleteConversion($id)
    {
        ...
        if($order)
        {             
            return redirect()->route('saleFreaks3bce')->withStatus(__('Order successfully deleted.'));
        }
        else
        {
            return redirect()->route('saleFreaks3bce')->withStatus(__('Order could not be deleted.'));
        }
    ...
    public function storeSettings(Request $request)
    {
        ...
        if(empty($settings))
            ...
            'amountCheck'=>$pricecheck,'stores'=>json_encode($stores),'storesCheck'=>$storecheck, 'discount'=>$discount, 'maxPrice'=>$maxPrice ,'maxDailyOrder'=>$maxDailyOrder, 'maxDailyAmount'=>$maxDailyAmount,'dailyAmountCheck'=>$dailyamtcheck, 'dailyOrderCheck'=>$dailyordercheck,'name'=>'saleFreaks3','priority'=>$priority,'enabled'=>$enabled,'listCheck'=>$showinlist]);
        else
            settings::where('name','saleFreaks3')->where('id',$settings->id)->update(['minAmount'=>$minAmount,'maxAmount'=>$maxAmount,
            'quantityRangeCheck'=>$qtyrangecheck,'minQty'=>$minQty,'maxQty'=>$maxQty,'amountCheck'=>$pricecheck,'stores'=>json_encode($stores),'storesCheck'=>$storecheck, 'discount'=>$discount, 'maxPrice'=>$maxPrice,'maxDailyOrder'=>$maxDailyOrder, 'maxDailyAmount'=>$maxDailyAmount,'dailyAmountCheck'=>$dailyamtcheck, 'dailyOrderCheck'=>$dailyordercheck,'name'=>'saleFreaks3','priority'=>$priority,'enabled'=>$enabled,'listCheck'=>$showinlist]);

        Session::flash('success_msg', __('Settings successfully updated'));
        return redirect()->route('saleFreaks3Setting');
    ...
    public function search(Request $request)
    {
        ...
        if($route == 'saleFreaks3new')
        {            
             
            if(auth()->user()->role==1)
            {            

                $orders = orders::select()->where('flag','24')->where('status','unshipped')                
            ...
            elseif(auth()->user()->role==2)
            {
                ...
                $orders = orders::select()->where('flag','24')->where('status','unshipped')->whereIn('storeName',$strArray)
                ->where(function($test) use ($query){
                    $test->where('sellOrderId', 'LIKE', '%'.$query.'%');
                    $test->orWhere('buyerName', 'LIKE', '%'.$query.'%');
                })                
            ...
            {
            $orders = orders::select()->where('flag','24')->where('status','unshipped')->where('uid',auth()->user()->id)
            ->where(function($test) use ($query){
                $test->where('sellOrderId', 'LIKE', '%'.$query.'%');
                $test->orWhere('buyerName', 'LIKE', '%'.$query.'%');
            })                
            ->orderBy('assignDate', 'ASC')->paginate(100);
                ...
                $to = date("m/d/Y", strtotime($endDate));  
                $dateRange = $from .' - ' .$to;
                return view('SaleFreaks3.new',compact('flags','orders','stores','states','maxAmount','minAmount','maxPrice','search','route','dateRange'));
            
        }

        else if ($route=='saleFreaks3cancel')
        {
            if(auth()->user()->role==1)            
        {
            $orders = cancelled_orders::leftJoin('orders','cancelled_orders.order_id','=','orders.id')
        ...
        elseif(auth()->user()->role==2)
        {
            ...
            $orders = $orders->appends('searchQuery',$query)->appends('route', $route);
            return view('SaleFreaks3.cancel',compact('orders','search','route'));
        }        

        else if ($route=='saleFreaks3bce')
        {
        ...
        elseif(auth()->user()->role==2)
        {
            ...
            $orders = $orders->appends('searchQuery',$query)->appends('route', $route);
            return view('SaleFreaks3.bce',compact('orders','search','route'));
        }  

        else if ($route=='saleFreaks3processed')
        {
            if(auth()->user()->role==1)
            ...
            elseif(auth()->user()->role==2)
                {
                    ...
                    ->where('account_id','SaleFreaks3')->whereIn('storeName',$strArray)->orderBy('date', 'ASC')->paginate(100);
                }
                ...
                ->where('account_id','SaleFreaks3')->where('uid',auth()->user()->id)->orderBy('date', 'ASC')->paginate(100);
            ...
            $orders = $orders->appends('searchQuery',$query)->appends('route', $route);
            return view('SaleFreaks3.processed',compact('orders','search','route'));
        }  
        elseif($route=='saleFreaks3return')
        {
            ...
            return view('SaleFreaks3.return',compact('returns','accounts','stores','dateRange','search','route'));
            
            
        }
        elseif($route=='saleFreaks3refund')
        {
            if(auth()->user()->role==1)
            {
                $returns = returns::leftJoin('orders','orders.id','=','returns.order_id')
            ...
            $dateRange = $from .' - ' .$to;
            $returns = $returns->appends('searchQuery',$query)->appends('route', $route);
            return view('SaleFreaks3.refund',compact('returns','accounts','stores','dateRange','search','route'));
        }
        elseif($route=='saleFreaks3completed')
        {
            if(auth()->user()->role==1)
            {
                $returns = returns::leftJoin('orders','orders.id','=','returns.order_id')
            ...
            $dateRange = $from .' - ' .$to;
            $returns = $returns->appends('searchQuery',$query)->appends('route', $route);
            return view('SaleFreaks3.complete',compact('returns','accounts','stores','dateRange','search','route'));
        }
