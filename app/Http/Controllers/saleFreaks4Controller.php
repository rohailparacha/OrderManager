...
class saleFreaks4Controller extends Controller
{
    //
    ...
    {
        $stores = accounts::all();
        $settings = settings::where('name','salefreaks4')->get()->first();
        return view('saleFreaks4Settings',compact('stores','settings'));
    }
    ...
    public function saleFreaks4export(Request $request)
    {
        ...
        return Excel::download(new SaleFreaksExport($storeFilter,$marketFilter,$stateFilter,$amountFilter,$sourceFilter, $daterange,'25'), $filename);
    }

    public function saleFreaks4OrderExport(Request $request)
    {        
        $filename = date("d-m-Y")."-".time()."-autofulfill-orders.csv";
        return Excel::download(new SaleFreaksOrdersExport('25'), $filename);
    }
    ...
    {    
        if(auth()->user()->role==1)
            $orders = orders::select()->where('status','processing')->where('account_id','SaleFreaks4')->orderBy('date', 'ASC')->paginate(100);
        ...
        elseif(auth()->user()->role==2)
            {
                ...
                $orders = orders::select()->where('status','processing')->where('account_id','SaleFreaks4')->whereIn('storeName',$strArray)->orderBy('date', 'ASC')->paginate(100);
            }
        
        else
            
            $orders = orders::select()->where('status','processing')->where('account_id','SaleFreaks4')->where('uid',auth()->user()->id)->orderBy('date', 'ASC')->paginate(100);
        ...
        foreach ($orders as $order)
        {
            $bcecheck = orders::select()->where('converted',false)->where('account_id','SaleFreaks4')
        ...
        return view('SaleFreaks4.processed',compact('orders'));
    ...
    public function autoFulfillFilter(Request $request)
    {
        ...
        if(auth()->user()->role==1)
            $orders = $orders->where('flag','25')->where('status','unshipped')->orderBy('assignDate', 'ASC')->groupby('orders.id')->paginate(100);
        elseif(auth()->user()->role==2)
        {
            $stores = accounts::select()->where('manager_id',auth()->user()->id)->get(); 
                $strArray  = array();
                ...
                $orders = $orders->where('flag','25')->where('status','unshipped')->whereIn('storeName',$strArray)->orderBy('assignDate', 'ASC')->paginate(100);
        }
            
        else
            $orders = $orders->where('flag','25')->where('status','unshipped')->where('uid',auth()->user()->id)->orderBy('assignDate', 'ASC')->groupby('orders.id')->paginate(100);
        ...
        $maxPrice = ceil(orders::where('status','unshipped')->where('flag','25')->max('totalAmount'));
        foreach($orders as $order)
        {
        ...
        $flags= flags::select()->whereNotIn('id',['16','25','8','9','10'])->get();  
        $route = 'saleFreaks4new';
        return view('SaleFreaks4.new',compact('flags','orders','stateFilter','marketFilter','sourceFilter','storeFilter','amountFilter','stores','states','maxAmount','minAmount','maxPrice','dateRange','route'));
    }
    ...
    {  
            if(auth()->user()->role==1)
            {
                $orders = orders::select()->where('status','unshipped')->orderBy('assignDate', 'ASC')->where('flag','25')->paginate(100);
            }
            ...
            elseif(auth()->user()->role==2)
            {
                ...
                $orders = orders::select()->where('status','unshipped')->whereIn('storeName',$strArray)->where('flag','25')->orderBy('assignDate', 'ASC')->paginate(100);
        ...
        $maxAmount = ceil(orders::where('status','unshipped')->where('flag','25')->max('totalAmount'));
        ...
        $flags= flags::select()->whereNotIn('id',['16','25','8','9','10'])->get();  
        $startDate = orders::where('status','unshipped')->where('flag','25')->min('assignDate');
        $endDate = orders::where('status','unshipped')->where('flag','25')->max('assignDate');

        $from = date("m/d/Y", strtotime($startDate));  
        $to = date("m/d/Y", strtotime($endDate));  
        $dateRange = $from .' - ' .$to;
        return view('SaleFreaks4.new',compact('flags','orders','stores','states','maxAmount','minAmount','maxPrice','dateRange'));
    }
    ...
    {       
        ...
        if(auth()->user()->role==1)            
        {
            $orders = orders::select()->where('converted',false)->where('account_id','SaleFreaks4')
            ->where('marketPlace','Walmart')
            ->where('carrierName',$amzCarrier->id)
            ->where('status','processing')
            ->where('trackingNumber','like','TBA%')
        ...
        elseif(auth()->user()->role==2)
        {
            ...
            ->where('marketPlace','Walmart')
            ->where('carrierName',$amzCarrier->id)
            ->where('account_id','SaleFreaks4')->whereIn('storeName',$strArray)
            ->where('status','processing')
            ->where('trackingNumber','like','TBA%')
            ->orderBy('status', 'DESC')->paginate(100);
        ...
        return view('SaleFreaks4.bce',compact('orders'));
    }
    ...
    public function updateSheetTracking($tracking, $sellOrderId, $carrier)
    {
        ...
        try{
            ...
            $client = new client(); 
            $endPoint = env('SALEFREAKS4_TOKEN', '');
    ...
    public function autoFulfillProcess(Request $request)
    {
        ...
        if($request->hasFile('file'))
        {
            ...
            {                
           ...
           else
             {
                ...
                return redirect()->route('saleFreaks4new');
             }
        ...
        $import = new SaleFreaksImport('25');
    ...
    {        
        ...
        return view('SaleFreaks4.cancel',compact('orders'));
    }
    ...
    public function deleteCancelled($id)
    {
        ...
        if($order->poNumber != $order->afpoNumber)
        {
            ...
            return redirect()->route('saleFreaks4cancel')->withStatus(__('Order successfully deleted.'));
        }
        else
        {
            return redirect()->route('saleFreaks4cancel')->withStatus(__('Order could not be deleted.'));
        }
    ...
    public function deleteConversion($id)
    {
        ...
        if($order)
        {             
            return redirect()->route('saleFreaks4bce')->withStatus(__('Order successfully deleted.'));
        }
        else
        {
            return redirect()->route('saleFreaks4bce')->withStatus(__('Order could not be deleted.'));
        }
    ...
    public function storeSettings(Request $request)
    {
        ...
        if(empty($settings))
            ...
            'amountCheck'=>$pricecheck,'stores'=>json_encode($stores),'storesCheck'=>$storecheck, 'discount'=>$discount, 'maxPrice'=>$maxPrice ,'maxDailyOrder'=>$maxDailyOrder, 'maxDailyAmount'=>$maxDailyAmount,'dailyAmountCheck'=>$dailyamtcheck, 'dailyOrderCheck'=>$dailyordercheck,'name'=>'saleFreaks4','priority'=>$priority,'enabled'=>$enabled,'listCheck'=>$showinlist]);
        else
            settings::where('name','saleFreaks4')->where('id',$settings->id)->update(['minAmount'=>$minAmount,'maxAmount'=>$maxAmount,
            'quantityRangeCheck'=>$qtyrangecheck,'minQty'=>$minQty,'maxQty'=>$maxQty,'amountCheck'=>$pricecheck,'stores'=>json_encode($stores),'storesCheck'=>$storecheck, 'discount'=>$discount, 'maxPrice'=>$maxPrice,'maxDailyOrder'=>$maxDailyOrder, 'maxDailyAmount'=>$maxDailyAmount,'dailyAmountCheck'=>$dailyamtcheck, 'dailyOrderCheck'=>$dailyordercheck,'name'=>'saleFreaks4','priority'=>$priority,'enabled'=>$enabled,'listCheck'=>$showinlist]);

        Session::flash('success_msg', __('Settings successfully updated'));
        return redirect()->route('saleFreaks4Setting');
    ...
    public function search(Request $request)
    {
        ...
        if($route == 'saleFreaks4new')
        {            
             
            if(auth()->user()->role==1)
            {            

                $orders = orders::select()->where('flag','25')->where('status','unshipped')                
            ...
            elseif(auth()->user()->role==2)
            {
                ...
                $orders = orders::select()->where('flag','25')->where('status','unshipped')->whereIn('storeName',$strArray)
                ->where(function($test) use ($query){
                    $test->where('sellOrderId', 'LIKE', '%'.$query.'%');
                    $test->orWhere('buyerName', 'LIKE', '%'.$query.'%');
                })                
            ...
            {
            $orders = orders::select()->where('flag','25')->where('status','unshipped')->where('uid',auth()->user()->id)
            ->where(function($test) use ($query){
                $test->where('sellOrderId', 'LIKE', '%'.$query.'%');
                $test->orWhere('buyerName', 'LIKE', '%'.$query.'%');
            })                
            ->orderBy('assignDate', 'ASC')->paginate(100);
                ...
                $to = date("m/d/Y", strtotime($endDate));  
                $dateRange = $from .' - ' .$to;
                return view('SaleFreaks4.new',compact('flags','orders','stores','states','maxAmount','minAmount','maxPrice','search','route','dateRange'));
            
        }

        else if ($route=='saleFreaks4cancel')
        {
            if(auth()->user()->role==1)            
        {
            $orders = cancelled_orders::leftJoin('orders','cancelled_orders.order_id','=','orders.id')
        ...
        elseif(auth()->user()->role==2)
        {
            ...
            $orders = $orders->appends('searchQuery',$query)->appends('route', $route);
            return view('SaleFreaks4.cancel',compact('orders','search','route'));
        }        

        else if ($route=='saleFreaks4bce')
        {
        ...
        elseif(auth()->user()->role==2)
        {
            ...
            $orders = $orders->appends('searchQuery',$query)->appends('route', $route);
            return view('SaleFreaks4.bce',compact('orders','search','route'));
        }  

        else if ($route=='saleFreaks4processed')
        {
            if(auth()->user()->role==1)
            ...
            elseif(auth()->user()->role==2)
                {
                    ...
                    ->where('account_id','SaleFreaks4')->whereIn('storeName',$strArray)->orderBy('date', 'ASC')->paginate(100);
                }
                ...
                ->where('account_id','SaleFreaks4')->where('uid',auth()->user()->id)->orderBy('date', 'ASC')->paginate(100);
            ...
            $orders = $orders->appends('searchQuery',$query)->appends('route', $route);
            return view('SaleFreaks4.processed',compact('orders','search','route'));
        }  
        elseif($route=='saleFreaks4return')
        {
            ...
            return view('SaleFreaks4.return',compact('returns','accounts','stores','dateRange','search','route'));
            
            
        }
        elseif($route=='saleFreaks4refund')
        {
            if(auth()->user()->role==1)
            {
                $returns = returns::leftJoin('orders','orders.id','=','returns.order_id')
            ...
            $dateRange = $from .' - ' .$to;
            $returns = $returns->appends('searchQuery',$query)->appends('route', $route);
            return view('SaleFreaks4.refund',compact('returns','accounts','stores','dateRange','search','route'));
        }
        elseif($route=='saleFreaks4completed')
        {
            if(auth()->user()->role==1)
            {
                $returns = returns::leftJoin('orders','orders.id','=','returns.order_id')
            ...
            $dateRange = $from .' - ' .$to;
            $returns = $returns->appends('searchQuery',$query)->appends('route', $route);
            return view('SaleFreaks4.complete',compact('returns','accounts','stores','dateRange','search','route'));
        }
